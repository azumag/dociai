...
test("Electron renderer adapters route every external service through Main IPC", async () => {
  const [connectors, news, topics, voicevox, bouyomi, runtimeFactory, platform, preload, ipc, main] = await Promise.all([
    source("src/connectors.js"), source("src/news-reader.js"), source("src/topic-reader.js"), source("src/voicevox.js"), source("src/bouyomi.js"), source("src/app/runtime-factory.js"), source("src/platform/electron-services.js"), source("electron/preload/index.ts"), source("electron/main/ipc/register.ts"), source("electron/main/index.ts"),
  ]);
