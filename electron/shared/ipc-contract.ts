...
export type SecretStatus = { key: string; configured: boolean };
...
export type DociaiApi = {
  ...
  config: {
    get(): Promise<Result<Record<string, unknown>>>;
    save(input: Record<string, unknown>): Promise<Result<{ saved: false }>>;
  };
  secrets: {
    status(keys?: string[]): Promise<Result<SecretStatus[]>>;
    set(input: { key: string; value: string }): Promise<Result<{ saved: false }>>;
    remove(key: string): Promise<Result<{ removed: false }>>;
  };
  windows: {
