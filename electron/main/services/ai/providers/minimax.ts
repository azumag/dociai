  ...
  try {
    while (true) {
      ...
      for (const line of lines) {
        ...
        let event: { delta?: { text?: unknown }; usage?: unknown; content_block?: { text?: unknown } };
...
export async function miniMaxChat(fetchFn: typeof fetch, config: { id: string; model: string; baseUrl: string; apiKey: string }, messages: AiMessage[], options: ChatOptions, signal: AbortSignal): Promise<{ text: string; usage: unknown }> {
