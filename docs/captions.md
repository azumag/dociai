...
`maxCaptionChars` を設定して1件の字幕が複数に分割される場合、分割数が `maxPending` を超えると
収まらない末尾が破棄されます (文の先頭と読み順は保たれます)。分割を使うときは `maxPending` を
想定する分割数以上にしてください。
...
- 翻訳1件あたりのタイムアウト (`caption-worker.js` の `TRANSLATE_TIMEOUT_MS`, 既定10秒) が
  通常の翻訳所要時間に対して妥当か。短すぎると遅い回線・端末で正常な翻訳まで捨ててしまい、
  長すぎると翻訳が固まったときに字幕が長時間止まる
