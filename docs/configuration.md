...
「発話中」判定になると `SpeechQueue.hold("mic")` が呼ばれます。**通常の読み上げ
(コメント読み上げ/AI応答) は再生中の発話を中断せず最後まで読み上げ** ますが、
**話題/ニュースの長尺読み上げは即時中断してキュー先頭へ戻り**、無音復帰後に
最初から読み直されます。いずれも次に控えている項目の開始は保留され、無音判定に戻ると
`SpeechQueue.release("mic")` が呼ばれ、保留していた次の項目の読み上げが始まります
(手動の「停止」ボタンは全ての再生を即時中断します)。しきい値の調整は操作卓の
「マイク監視」パネルのメーター表示を見ながら行ってください。
...
`news.enabled: true` の間は、操作卓の「ニュース」パネルにある「自動読み上げ」トグルで
自動読み上げ・トリガー発火をセッション中だけON/OFFできます (config保存不要・即時反映)。
`news.enabled: false` のときはこのトグル自体が無効化されます。
...
| `randomPersona` | `true` で `persona` の代わりに `personas` からランダムに1体を毎回抽選する。既定 `false` |
| `personas` | `randomPersona: true` のときの抽選候補ペルソナIDの配列。話題ごと (item単位) に抽選し直すため、同じ実行内でも担当が変わりうる。無効化されたペルソナや空配列の場合は `persona` → `router.defaultPersona` にフォールバックする |
...
`topics.enabled: true` の間は、操作卓の「話題」パネルにある「自動読み上げ」トグルで
自動読み上げ・トリガー発火をセッション中だけON/OFFできます (config保存不要・即時反映)。
`topics.enabled: false` のときはこのトグル自体が無効化されます。
