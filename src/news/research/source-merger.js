...
// 話題が重なる (token 2件以上共有) 2 factのうち、数値集合が互いにsubsetでない (どちらか一方
// だけが他方の情報を包含しているのではなく、本当に食い違う数値がある) 場合だけを対立claimと
// 見なす。片方が「死者12人」もう片方が「死者12人、負傷3人」のような包含関係は対立ではない。
function detectConflicts(facts) {
  const conflicts = [];
  for (let i = 0; i < facts.length; i++) {
    const a = facts[i];
    const aTokens = new Set(significantTokens(a.text));
    const aNumbers = new Set(extractNumbers(a.text));
    for (let j = i + 1; j < facts.length; j++) {
      const b = facts[j];
      const bTokens = significantTokens(b.text);
      const shared = bTokens.filter((token) => aTokens.has(token));
      if (shared.length < 2) continue;
      const bNumbers = new Set(extractNumbers(b.text));
      if (!aNumbers.size || !bNumbers.size) continue;
      const aSubsetOfB = [...aNumbers].every((n) => bNumbers.has(n));
      const bSubsetOfA = [...bNumbers].every((n) => aNumbers.has(n));
      if (!aSubsetOfB && !bSubsetOfA) conflicts.push(`「${a.text}」と「${b.text}」で数値が食い違っています`);
    }
  }
  return conflicts;
}

function dedupeFacts(facts, hostById) {
  ...
  for (const fact of facts) {
    ...
    // 独立した (=別publisherの) 複数sourceが同じ事実を裏付けた場合だけconfidenceを一段
    // 引き上げる。同一publisherの転載2件は独立source数へ数えない (ファイル冒頭の不変条件)。
    const distinctHosts = new Set(existing.sourceIds.map((id) => hostById.get(id)).filter(Boolean));
    if (distinctHosts.size >= 2 && existing.confidence !== "high") existing.confidence = "high";
  }
...
export function mergeProviderResults(candidateId, headline, providerResults) {
  ...
  const hostById = new Map([...sourcesByHash.values()].map((s) => [s.id, canonicalHost(s.url)]));
  const facts = dedupeFacts(factsByKind.fact, hostById).filter((f) => f.text);
  ...
  for (const conflict of conflicts) unresolved.add(conflict);
  ...
  return {
    ...
    coverage: {
      ...
      // 数値の食い違いという具体的なconflictだけをここへ反映する。provider由来の一般的な
      // unresolved note (「発表時刻が未確認」等) はunresolvedへは残すが、conflictとしては
      // 数えない (unresolved.size > 0 で判定すると無関係な注記までconflict扱いになるため)。
      hasConflictingClaims: conflicts.length > 0,
    },
  };
}
