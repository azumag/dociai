...
export function validateConfigStructure(config) {
  ...
  for (const [id, connector] of Object.entries(config.connectors ?? {})) {
    if (!registryIds("providers").includes(connector?.provider)) issues.push(issue(["connectors", id, "provider"], "enum", "Unsupported provider", { meta: { options: registryIds("providers") } }));
    if (connector?.opencodeSession !== undefined && typeof connector.opencodeSession !== "boolean") issues.push(issue(["connectors", id, "opencodeSession"], "type.boolean", "opencodeSession must be a boolean"));
  }
