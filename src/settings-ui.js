...
export class SettingsUI {
  ...
  #renderMicMonitor() {
    ...
    const note = document.createElement("p");
    note.className = "muted settings-note";
    note.textContent = "配信者の発話を検知すると次のAI音声の開始を保留し、無音に戻ると再開します。通常の読み上げは再生中の項目を止めずに最後まで続けますが、話題/ニュースの長尺読み上げは即時中断して無音復帰後に最初から読み直します。スピーカー環境ではAI自身の声を誤検知することがあるため、ヘッドホンや仮想オーディオデバイスでの分離を推奨します (docs/obs-mode.md 参照)。deviceが「既定のデバイス」以外に一覧されない場合は、一度「監視開始」でマイク権限を許可してから設定を開き直してください。";
    this._body.append(note);
  }
