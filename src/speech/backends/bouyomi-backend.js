...
const CHARS_PER_SECOND = 6; // speed=100 (棒読みちゃん既定) 相当のおおよその発話速度
const MIN_SPEAK_MS = 400;
const MAX_SPEAK_MS = 60_000;

export function estimateBouyomiSpeakMs(text, speed) {
  const rate = Number(speed) > 0 ? Number(speed) / 100 : 1;
  const ms = (String(text ?? "").length / (CHARS_PER_SECOND * rate)) * 1000;
...
export class BouyomiBackend extends SpeechBackend {
  constructor(client, { onHealth = () => {}, wait = waitOrAbort } = {}) {
  ...
  async play(item, context = {}) {
    ...
    try {
      ...
      await this.wait(estimateBouyomiSpeakMs(item.text, item.voice?.speed ?? item.voice?.rate), controller.signal);
